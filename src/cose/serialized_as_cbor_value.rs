...
impl<'a, T: Clone + AsCborValue> Serialize for SerializedAsCborValue<&'a T> {
    ...
    where
        S: serde::Serializer,
    {
        self.0.clone().to_cbor_value().map_err(serde::ser::Error::custom)?.serialize(serializer)
    }
}
...
impl<'de, T: AsCborValue> Deserialize<'de> for SerializedAsCborValue<T> {
    ...
    {
        T::from_cbor_value(ciborium::Value::deserialize(deserializer)?).map_err(serde::de::Error::custom).map(Self)
    }
}
