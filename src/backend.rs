...
// TODO: QR CODE
// TODO: currently returns JSON string of URI, return QR code more directly
#[get("/generate_qr_code")]
async fn qr_code_api(request: web::Form<RequestQR>) -> impl Responder {
    HttpResponse::Ok().json(request.uri().map(|x| x.to_string()))
}
...
// If the OIDC4VCI Token Request was unsuccessful, e.g., invalid or expired pre-authorized_code, multiple use of pre-authorized_code, invalid PIN etc., the DMV Service returns an error response as defined below.
//
...
// - After the mDL App successfully obtains an access_token from the OIDC4VCI token endpoint, the mDL app sends an OIDC4VCI Credential Request to the OIDC4VCI credentials endpoint.
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
...
// Table: Details of OIDC4VCI Proof of Possession
#[derive(Clone, Debug, Serialize, Deserialize)]
