...
mod tests {
    ...
    use crate::cose::sign1::{Error, PreparedCoseSign1, VerificationResult};
    use crate::cose::{mac0, MaybeTagged, SignatureAlgorithm};
    use coset::{iana, CoseMac0};
    use digest::Mac;
    use hex::FromHex;
    use hmac::Hmac;
    ...
    fn test_serialization_deserialization() {
        ...
        // Serialize to CBOR
        let serialized = cbor::to_vec(&mcd).unwrap();
        println!("Serialized MCD: {:?}", serialized);

        // Deserialize from CBOR
        let deserialized: MobileIdCapabilityDescriptor = cbor::from_slice(&serialized).unwrap();
    ...
    fn test_signing_cose_sign1_and_verification() {
        ...
        let cose_sign1_serialized = cbor::to_vec(&cose_sign1).unwrap();
        println!("Serialized COSE_Sign1: {:?}", cose_sign1_serialized);

        // Deserialize COSE_Sign1 from CBOR
        let cose_sign1_deserialized: MaybeTagged<CoseSign1> =
            cbor::from_slice(&cose_sign1_serialized).unwrap();
    ...
    fn test_tagging_cose_mac0_and_verification() {
        ...
        let cose_mac0_serialized = cbor::to_vec(&cose_mac0).unwrap();
        println!("Serialized COSE_Mac0: {:?}", cose_mac0_serialized);

        // Deserialize COSE_Sign1 from CBOR
        let cose_mac0_deserialized: MaybeTagged<CoseMac0> =
            cbor::from_slice(&cose_mac0_serialized).unwrap();
